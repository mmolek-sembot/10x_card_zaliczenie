...
export class OpenRouterService {
  ...
  private static readonly DEFAULT_SYSTEM_MESSAGE =
    'You are a helpful assistant that creates educational content.';
  ...
  constructor(config: OpenRouterConfig = {}) {
    ...
    this.defaultSystemMessage =
      config.defaultSystemMessage || OpenRouterService.DEFAULT_SYSTEM_MESSAGE;
  ...
  private createCacheKey(options: ChatOptions): string {
    ...
    const stableOptions = {
      ...
      parameters:
        options.parameters ||
        (options.parameterPreset
          ? getParameterPreset(options.parameterPreset)
          : this.defaultParameters),
    };
    ...
    if (responseFormat && responseFormat.type !== 'text') {
      try {
        ...
        throw new SchemaValidationError(
          `Failed to parse or validate JSON response: ${(error as Error).message}`
        );
      }
    }
  ...
  ): ReadableStream<ChatResponseChunk> {
