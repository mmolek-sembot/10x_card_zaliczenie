...
export const FlashcardPreview: React.FC<FlashcardPreviewProps> = ({
  ...
  const getStatusBadge = () => {
    if (isRejected) return <span className="bg-red-100 text-red-700 text-xs px-2 py-1 rounded">Odrzucona</span>;
    if (isAccepted) return <span className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded">Zaakceptowana</span>;
    if (status === 'edited') return <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded">Edytowana</span>;
